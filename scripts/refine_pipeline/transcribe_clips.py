#!/usr/bin/env python3
"""
transcribe_clips.py - Transcribe a single clip with Groq Whisper v3.

Given `channel_name`, `episode_id`, and an optional `clip_id`, this script:
1. Locates one clip or every MP4 inside data/channels/<channel>/<episode>/refined_clips.
...
4. Stores the verbose JSON response under refined_clips/transcripts/<clip>_whisper_v3.json.
...
Usage:
    python scripts/clip_pipeline/transcribe_clips.py CHANNEL EPISODE [CLIP_ID]
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Transcribe a clip with Groq Whisper large v3.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="?",
        help="Optional clip identifier (e.g. clip01). When omitted all *.mp4 clips are processed.",
    )
    parser.add_argument(
        "--channels-dir",
        type=Path,
...
def ensure_clip_path(clips_dir: Path, clip_id: str, extension: str) -> Path:
    """Return the resolved MP4 path for a clip id."""
...
def main() -> None:
    ...
    clip_root = channels_dir / args.channel_name / args.episode_id / "refined_clips"
    transcripts_dir = clip_root / "transcripts"
    transcripts_dir.mkdir(parents=True, exist_ok=True)

    try:
        clip_paths = list_clip_paths(clip_root, args.clip_id, args.clip_extension)
    except FileNotFoundError as exc:
        print(f"❌ {exc}")
        sys.exit(1)

    if not clip_paths:
        print(f"❌ No clips ending with {args.clip_extension} found in {clip_root}")
        sys.exit(1)

    print("=" * 60)
    print("CLIP TRANSCRIPTION (Groq Whisper v3)")
    print("=" * 60)
    print(f"📺 Channel:     {args.channel_name}")
    print(f"🎬 Episode:     {args.episode_id}")
    print(f"✂️  Clips:       {len(clip_paths)} target(s)")
    print(f"📁 Clips dir:   {clip_root.resolve()}")
    ...
    for clip_path in clip_paths:
        ...
        output_path = transcripts_dir / f"{clip_stem}_whisper_v3.json"
