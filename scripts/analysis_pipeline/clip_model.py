#!/usr/bin/env python3
"""
train_clip_model.py - CatBoost model for predicting clip performance

Trains two binary classifiers:
1. top25_vpi - Will this clip outperform channel baseline on reach?
2. top25_eng - Will this clip have above-average engagement?

Uses SHAP for interpretability.

Usage:
    python train_clip_model.py --input master_query_results.csv
...
def create_targets(df: pd.DataFrame) -> pd.DataFrame:
    """
    Create binary classification targets based on top 25% threshold.
    """
    print("\n" + "="*70)
    ...
    # Top 25% thresholds
    vpi_threshold = df['VPI'].quantile(0.75)
    eng_threshold = df['engagement_rate'].quantile(0.75)
    
    print(f"VPI threshold (75th percentile): {vpi_threshold:.3f}")
    print(f"Engagement threshold (75th percentile): {eng_threshold:.4f}")
    
    # Binary targets
    df['top25_vpi'] = (df['VPI'] >= vpi_threshold).astype(int)
    df['top25_eng'] = (df['engagement_rate'] >= eng_threshold).astype(int)
    
    print(f"\nTop 25% VPI clips: {df['top25_vpi'].sum()} ({df['top25_vpi'].mean()*100:.1f}%)")
    print(f"Top 25% Engagement clips: {df['top25_eng'].sum()} ({df['top25_eng'].mean()*100:.1f}%)")

        # --- Cyclic time encodings for hour/day ---
    # Expecting columns: day_published_number (0–6), hour_published_number (0–23)
    df["hour_sin"] = np.sin(2 * np.pi * df["hour_published_number"] / 24.0)
    df["hour_cos"] = np.cos(2 * np.pi * df["hour_published_number"] / 24.0)

    df["dow_sin"] = np.sin(2 * np.pi * df["day_published_number"] / 7.0)
    df["dow_cos"] = np.cos(2 * np.pi * df["day_published_number"] / 7.0)
...
def main():
    parser = argparse.ArgumentParser(description='Train clip performance prediction models')
    ...
    print("="*70)
    print("CLIP PERFORMANCE ML TRAINING")
    print("="*70)
    ...
    # Train-test split (single split to keep features aligned across both targets)
    stratify_labels = df[['top25_vpi', 'top25_eng']].agg(tuple, axis=1)
    ...
    y_train_vpi, y_test_vpi = df.loc[train_idx, 'top25_vpi'], df.loc[test_idx, 'top25_vpi']
    y_train_eng, y_test_eng = df.loc[train_idx, 'top25_eng'], df.loc[test_idx, 'top25_eng']
    
    # ========== MODEL 1: VPI (Reach) ==========
    model_vpi, metrics_vpi = train_model(
        X_train, y_train_vpi,
        X_test, y_test_vpi,
        cat_feature_indices,
        'top25_vpi'
    ...
    # Save model
    model_vpi.save_model(output_dir / 'model_vpi.cbm')
    print(f"\nSaved: model_vpi.cbm")
    
    # SHAP analysis
    shap_vpi, importance_vpi = run_shap_analysis(model_vpi, X, 'top25_vpi', output_dir)
    
    # Analyze top clips
    analyze_top_clips(model_vpi, X, df, 'top25_vpi')
    ...
    print("-"*60)
    print(f"{'VPI (Reach)':<20} {metrics_vpi['roc_auc']:<10.3f} {metrics_vpi['cv_mean']:<10.3f} {metrics_vpi['precision']:<10.3f} {metrics_vpi['recall']:<10.3f}")
    ...
    print(f"\nVPI Model:")
    for _, row in importance_vpi.head(5).iterrows():
        print(f"  {row['feature']:<25} {row['importance']:.4f}")
    ...
    print(f"  - model_vpi.cbm")
    print(f"  - model_eng.cbm")
    print(f"  - top25_vpi_shap_summary.png")
    print(f"  - top25_eng_shap_summary.png")
    print(f"  - *_feature_importance.csv")
    print(f"{'='*70}")
